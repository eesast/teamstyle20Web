...
#include <cmath>
	...
	void play_game()
	{
		...
		if (frame == 0)
		{
			...
			XYPosition landing_point = { 50 , 51 };
	...
	void new_pick_up() {
		...
		if (weapon_cnt() <= 3) {
			...
			{
				if (info.items[i].polar_pos.distance < closest_item.polar_pos.distance && info.items[i].type == WEAPON)
				{
			...
			for (int i = 0; i < info.items.size(); ++i)
			{
				if (info.items[i].polar_pos.distance < closest_item.polar_pos.distance && info.items[i].type == ARMOR)
				{
					closest_item = info.items[i];
					flag = true;
				}
