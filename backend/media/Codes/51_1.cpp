...
const std::string Name = "Signalman";
Item target_item;
ITEM weapon;
...
inline void Item_Check()
{
	...
	for (int i = 0; i < info.items.size(); ++i)
		if (info.items[i].polar_pos.distance < target_item.polar_pos.distance && info.items[i].type != CODE_CASE)
...
void play_game()
{
	...
	{
		srand((time(nullptr) * (info.player_ID + frame)) % 1007);
		XYPosition landing_point = { 250 + rand() % 10, 350 + rand() % 10 };
