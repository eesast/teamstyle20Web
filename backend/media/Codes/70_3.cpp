...
const double poisonRatio[VOCATION_SZ][3][3] = {//职业,时间段,第几个参数 
...
const int valueEarlier[ITEM_SZ][VOCATION_SZ] = {//枪较少，加大枪比重
...
int totalHurt, totalHeal;//当前身上武器能造成的总伤害
...
double Astar::RealDist(XYPosition d)//若所有路都在圈内则返回距离，否则返回距离负值
{
...
void CostProcess(int part)//共10部分，传入12345678910
{
	const int r = 3;
	static int count;
	int start, end;
...
bool CircIntersect(double ang, double x0, double y0, double r, double critical)//ang为绝对角度，坐标均需传入以人所在处为原点相对坐标
{
...
bool Intersect(XYPosition og, double ang, double bias, double critical = 0.6)//ang为绝对角度
{
	XYPosition tmp;
	tmp.x = tmp.y = 50.0;
	for (int y = 9; y >= 0; y--)
...
void Initial(VOCATION profession)//待完善
{
	...
	if (frame == 0)
	{
		...
		case 0:
			...
			case MEDIC:
				...
				break;
			case HACK:
				//jump.x = 735, jump.y = 350;//hill靠下
				jump.x = 565, jump.y = 430;
				break;
			case SNIPER:
				//jump.x = 455, jump.y = 780;//hill靠上
				jump.x = 435, jump.y = 430;
				break;
			default://signalman
				//jump.x = 355, jump.y = 550;//左侧草丛
...
XYPosition MyPace(double moveAng, double length)//moveAng为绝对角度，刚捡完东西的RELAX状态会预测错误
{
...
bool Move(double move_angle, double view_angle, int strong = 0, int parameter = -1)//参数：前进方向与视角的相对角度（相对于当前视角），parameter == NOMOVE(0)时不移动，只调整角度
{
	if (parameter != NOMOVE)
	{
		switch (nowStatus)
...
bool Reachable(XYPosition p)//仅供Shrink使用！！！！！！！！比landform判定要严！！！！
{
...
bool Priority(ITEM a, ITEM b, double dista, double distb, int lock = 0)//1: b nb, 0: a nb, lock=1:已锁定
{
	int category[2];
	if (dista < 0 && distb > 0)
		return 1;
...
int FindGarbageNum()//返回vector编号，没有返回-1
{
...
bool CanSeeMe(int num)//传入敌人编号，返回是否能看到自己
{
...
int FindEnemyNumRange(double lmin, double lmax, bool canSee)//在距离为lmin到lmax内寻找最近的敌人（canSee：是否优先找看见自己的）
{
	int num = -1;
...
bool DoFirstThing()
{
	...
	if (!path.empty())
	{
		...
		//if (distMin/*Dist(f.destination, info.self.xy_pos)*/ <= 5.0 && f.mode == 2)//只有CHASE_ENEMY才会更改id
		//	idealMove = AngleLimit(idealMove - 30.0);
		//else if (distMin/*Dist(f.destination, info.self.xy_pos)*/ <= 5.0 && f.mode == 3)
		//	idealMove = AngleLimit(idealMove + 30.0);

		if (follow == 1)//转头，否则看终点
			idealView = 100.0;
		return Move(idealMove, idealView, strong);
	}
	else
...
void Heal()
{
	...
	{
		if ((!todo.empty() && todo.top().type != SHOOT) || todo.empty())//注意这里使用的是todoPrint
		{
			for (int i = 0; i < info.self.bag.size(); i++)
			{
...
XYPosition Grass()//返回-1，-1或圈内的最近草丛位置
{
	XYPosition center, tmp;//center存储最近的草丛中心位置
	center.x = center.y = -1.0;
	double distMin = 150.0;//最远检测距离自己150距离内的草丛
	if (!((frame > 200 && totalHurt >= 1000 && totalHeal >= 50) || (frame > 540 && totalHurt >= 300) || (frame > 1080 && totalHurt >= 200)))//不满足蹲草丛条件
		...
		for (int x = 0; x < 10; x++)
		{
			...
			for (int i = 0; i < AREA_DATA[blockType].size(); i++)
			{
				if (AREA_DATA[blockType][i].type == RECTANGLE_GRASS)
				{
					...
					{
						tmp.y = offsety + (double)yy;//长条草地分成四格，长条草地从10到90
	...
	if (center.x >= 0.0 && center.x <= 999.0 && center.y >= 0.0 && center.y <= 999.0)//蹲草丛
	{
