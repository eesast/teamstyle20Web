...
void YYX()//跑毒，给点随机算法可优化，现在有点暴力，改写别忘了必须给能到达的点
{
//	BFS finder;
	/*if (frame <= 210)
	{
	...
	if (inside == 0)
	{
		destination = info.poison.current_center;//finder.SearchAccess(info.poison.current_center);//需要BFS
		return;
	}
	else if (inside == 1)
	{
		destination = info.poison.next_center;//finder.SearchAccess(info.poison.next_center);//需要BFS
		return;
	}
	else//以下为inside == 2
	{//进来一定return
	...
	{//进来一定return
		...
		if (collectGarbage <= 0 && seeEnemy <= 1)//不想去找人/东西
		{
			...
			XYPosition org;
			org.x = org.y = 500;
			if (!(Dist(destination, org) < radius && Dist(destination, org) > 0.6 * radius && Dist(destination, info.self.xy_pos) > 15.0 && Reachable(destination) && Reachable(destination)))//des不满足（des位于地图中心的半径小于200且大于120的圆环内且距自己的距离大于15且能走）
			{
	...
	if (inside == 0)
	{
		destination = info.poison.current_center;//需要BFS
		return;
	}
	else if (inside == 1)
	{
		destination = info.poison.next_center;//需要BFS
		return;
	}
	else//以下为inside == 2
	{//进来一定return
